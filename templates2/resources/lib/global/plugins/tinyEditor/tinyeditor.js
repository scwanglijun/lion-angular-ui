/**
 * @modify 阳葵 增加对象实例缓存功能，解决在弹出窗口使用时，无法获取到对象实例的问题。
 *              解决同一个页面下有多个实例，只有最后一个实例可用的问题
 *
 * @type {{}}
 */
...
TINY.object = {};
//提供全局的方法获取对象
TINY.get = function (id) {
    return TINY.object[id];
};
//直接获取内容
TINY.getContent = function (id) {
    return TINY.object[id].toggle(1);
};
TINY.editor = function (n, obj) {
    var editorId = obj.id;
    var c = [], offset = -30;
    c['cut'] = [1, 'Cut', 'a', 'cut', 1];
    ...
    this.hover = function (div, pos, dir) {
        div.style.backgroundPosition = (dir ? '34px ' : '0px ') + (pos) + 'px'
    };
    this.ddaction = function (dd, a) {
        var i = dd.selectedIndex, v = dd.options[i].value;
        this.action(a, v)
    };
    this.action = function (cmd, val, ie) {
        if (ie && !this.ie) {
    ...
    this.insert = function (pro, msg, cmd) {
    ...
    //缓存对象，方便在不同的controller当中获取
    TINY.object[editorId] = this;
};
